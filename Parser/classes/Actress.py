...
class Actress(DataSet):
    def __init__(self):
        super().__init__()
        self.regex = r"^(?:, )?(?P<nick_name>\'[\S ]+?\'|^\"[\S ]+?\")?(?:,)?(?: )?(?:(?:(?P<last_name>[\S ]+?)?, )?(?P<first_name>[\S ]+?)[^ \S]+|(?:\t\t\t))\"?(?P<show_title>.+(?= \(Music Video\) \([\d?])|(?<=\").+?(?=\")|.+(?= \([\d?]))\"?(?:\s\((?P<music_video>Music Video)?\))?(?:\s\((?P<release_date>\d[^?]+?)\)|\s\(\?{4}.*?\))?(?:\s\((?P<type_of_show>TV|V|VG)\))?(?:\s\{(?P<episode_title>(?:(?!\(\#|\{).+?(?= \(#)|(?!\(\#|\{).+?(?=\}))?))?(?:\})?\s?(?:\(\#(?P<season_number>\d+?)\.(?P<episode_number>\d+?)\)\})?(?:\s?\{\{?(?P<suspended>SUSPENDED)\}\})?(?:(?:(?:(?:\s+)?\[(?:(?P<character_name>[^\[{()}\]]+?)?)(?:(?=\(|\{)|\)|\}|\])+)|(?:(?:\s+)?(?:\(|\{|\[)(?:(?:as (?P<also_known_as>[^)}\]]+?))|(?P<segment>segment[^)}\]]*?)|(?P<voice_actor>voice[^)}\]]*?)|(?P<scenes_deleted>scenes deleted)|(?P<credit_only>credit only)|(?P<archive_footage>archive footage)|(?P<uncredited>uncredited)|(?P<rumored>rumored)|(?P<motion_capture>motion capture)|(?:[^)}\]]+?))(?:\)|\}|\])+))+)?(?:\s+<(?P<role_position>[0-9]{1,})\>)?"
        self.file = "actresses"
        ...
        try:
            ...
            with conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT DISTINCT * from actresses WHERE episode_title IS NULL AND episode_number IS NULL AND season_number IS NULL")
                    data = cur.fetchall()
                    return data
            ...
            with conn:
                with conn.cursor() as cur:
                    ...
                    command = """
                              ...
                              WHERE show_info.show_info_id IS NOT NULL AND temp.episode_title IS NULL AND temp.season_number IS NULL AND temp.episode_number IS NULL
                              """
                    cur.execute(command)
                    data = cur.fetchall()
                    execute_values(cur,
                                   ...
                                   data)
                    Actress.insert_also_known_as(cur)
                    print("\033[1;32mFinished inserting Actresses to Role and Person")
