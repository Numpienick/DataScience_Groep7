...
class RunningTime(DataSet):
    def __init__(self):
        super().__init__()
        self.regex = r"(?P<Title>.*?)\((?P<Year>[0-9\/I\?]{4,8})\)\s+?(?:\{)?(?:\{(?P<EpisodeTitle>(?:(?!\(\#|\{).+?(?= \()|(?!\(\#|\{).+?(?=\}))?))?(?:\})?(?:\s)?(?:\(\#(?P<SeasonNumber>\d+?)\.(?P<EpisodeNumber>\d+?)\)\})?(?:\s)?(?:\0+?)?(?:\s+?)?\s+(?:\()?(?P<Showtype>(?:[TVG]{1,2})?)(?:\))?\s*?(?P<Country>(?:\w.*?\:)?)(?P<RunningTime>\d{1,4})\s+?(?:\()?(?P<Approx>(?:approx.)?)(?:\)\s*\()?(?P<Season>(?:\w*\s*season)?)(?:\)\s*\()?(?P<Commercials>(?:\w*\s*commercials)?)(?:\)\s*\()?(?P<TotalEpisodes>(?:\d{1,3}\s*episodes)?)(?:\)\s*\()?(?P<FPS>(?:\d*\s*fps)?)(?:\)\s*\()?(?P<Version>(?:[a-zA-Z0-9\s]*?version)?)(?:\)\s*\()?(?P<Parts>(?:\d{0,3}\s*parts|\w*\s*parts)?)(?:\)\s*\()?(?P<Festival>(?:[a-zA-z\s]*?Festival)?)(?:\)\s*\()?(?P<Cut>(?:[a-zA-z'\s]*?cut|[a-zA-z'\s]*?Cut)?)(?:\)\s*\()?(?P<Market>(?:[a-zA-z\s]*?Market)?)(?:\)\s*\()?(?P<Print>(?:\w*\s*print)?)(?:\))?"
